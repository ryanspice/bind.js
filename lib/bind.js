// re-author: Ryan Spice
// license: http://rem.mit-license.org/
// source: https://github.com/ryanspice/bind.js
//
//
//
...
const global = window; // support check
...
//for each override memory safe? ?

const forEach = (subject, fn)=>{

  //let verifiedChecks= new Set(subject); ////// BREAKS IE 11, SILENT

  let convertToArray =Array.from(subject);
  let length = convertToArray.length;

  for (let i = 0; i < length; i++) {
    fn(convertToArray[i], i, convertToArray);
  }
...
class Bind {
  ...
  constructor(obj,mapping,context){
      ...
      if (this.settings.deferred.length) {
        ...
        forEach(this.settings.deferred, function deferreds(fn) {
          fn();
        });
      }
      console.log(this);
  }
  __export = ()=>{ __export({},this);}
}
